...
setwd("/home/liyinong/permutationTest/female")
...
for (comb_id in start_id:end_id) {
  ...
  f_ca_mat <- f_ca_obj@net$prob
  f_ct_mat <- f_ct_obj@net$prob
  
  global_female_control <- compute_global_strength(f_ct_mat)
  global_female_mdd <- compute_global_strength(f_ca_mat)
  
  female_attenuation <- ifelse(global_female_control != 0, (1 - global_female_mdd / global_female_control) * 100, 0)
  #########################result1
  message("####################female_attenuation####################",female_attenuation)
  attenuation <- data.frame(
    combination_id = comb_id,
    attenuationValue = female_attenuation
  )
  write.csv(attenuation, file = "attenuation.csv", row.names = FALSE)

  
############################################step2####################################################  
  object.list <- list(Female_control = f_ct_obj, Female_MDD = f_ca_obj)
  cellchat <- mergeCellChat(object.list, add.names = names(object.list))
  # rm(object.list)
  
  # ref: scatter-for permutation.R
  netAnalysis_signalingRole_scatter_V2(cellchat, data=c("Female_control","Female_MDD"), F_flag = T, M_flag= F,external_objects = object.list)
  
############################################step5####################################################  
  female_case_pathways <- f_ca_obj@netP$pathways
  female_control_pathways <- f_ct_obj@netP$pathways
  all_pathways <- unique(c(female_case_pathways, female_control_pathways))
  ...
  female_case_sums <- calculate_pathway_sums(f_ca_obj)
  female_control_sums <- calculate_pathway_sums(f_ct_obj)
  
  result_df <- data.frame(pathway = all_pathways)
  
  
  result_df$female_case <- sapply(all_pathways, function(p) {
    if (p %in% names(female_case_sums)) female_case_sums[p] else 0
  })
  
  result_df$female_control <- sapply(all_pathways, function(p) {
    if (p %in% names(female_control_sums)) female_control_sums[p] else 0
  })
  
  
  result_df$female_diff <- 1 - (result_df$female_case / result_df$female_control)
  result_df$female_diff[!is.finite(result_df$female_diff)] <- NA
  result_df$caseMinuscontrol <- result_df$female_case - result_df$female_control
  print(head(result_df))
  write.csv(result_df, "venn_dif_3.csv", row.names = FALSE)
  
  
  rm(f_ct_obj,f_ca_obj)
  gc()
...
############################################step3####################################################   
  ...
  netAnalysis_contribution_v2(cellchat, dataset = c("Female_control","Female_MDD"), signaling = "NRXN")
  setwd("..")
  gc()
}
